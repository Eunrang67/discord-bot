...
#curse = ["시발", "씨발", "병신", "애미", "느금마", "너검마", "좆", "개새끼", "씹", "븅신"]
...
async def on_member_remove(member):
    syschannel = member.guild.system_channel.id 
    try:
        await client.get_channel(syschannel).send(f"{member}님이 {member.guild} 서버에서 퇴장하셨습니다!\n현재 인원수 : {str(len(member.guild.members))}")
    except:
        return None
...
#            embed = discord.Embed(title=f"욕설 사용", color=0xFF0000, timestamp=message.created_at)
#            embed.add_field(name="보낸 메시지", value=f"```{message_content}```", inline=False)
#            embed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
#            await message.channel.send(embed=embed)
#            await message.delete()

    if message.content  == ">도움말" or message.content  == ">명령어" or message.content  == ">도움":
        embed = discord.Embed(title=f"명령어 도움말", color=0xFFFF00, timestamp=message.created_at)
        embed.add_field(name="미리 알아두세요!", value=f"< >필드는 필수, [ ] 필드는 선택입니다", inline=False)
        embed.add_field(name=">추방 <@user> [사유]", value=f"멘션한 유저를 서버에서 추방합니다\n`유저 추방 권한 필요`", inline=False)
        embed.add_field(name=">차단 <@user> [사유]", value=f"멘션한 유저를 서버에서 차단합니다\n`유저 차단 권한 필요`", inline=False)
        ...
        embed.add_field(name=">삭제 <숫자>", value=f"입력한 숫자만큼 메시지를 삭제합니다\n`메시지 관리 권한 필요`", inline=False)
        ...
        embed.add_field(name=">경고 <@user> [사유]", value=f"멘션한 유저에게 경고 1을 부여합니다\n`감사로그 보기 권한 필요`", inline=False)
        embed.add_field(name=">경고차감 <@user>", value=f"멘션한 유저에게 경고 1을 차감합니다\n`감사로그 보기 권한 필요`", inline=False)
        embed.add_field(name=">경고확인 <@user>", value=f"멘션한 유저의 경고받은 횟수를 확인합니다", inline=False)
        embed.add_field(name=">경고초기화 <@user>", value=f"멘션한 유저의 경고횟수를 초기화합니다\n`감사로그 보기 권한 필요`", inline=False)
        embed.add_field(name=">인증역할 <역할이름>", value=f"`>인증` 명령어의 역할을 설정합니다\n`관리자 권한 필요`", inline=False)
        embed.add_field(name=">인증역할제거", value=f"`>인증` 명령어의 역할을 제거합니다\n`관리자 권한 필요`", inline=False)
        embed.add_field(name=">인증", value=f"캡챠로 사람임을 인증합니다(?)", inline=False)
        embed.add_field(name=">뮤트역할", value=f"`>뮤트` 명령어의 역할을 설정합니다\n`관리자 권한 필요`", inline=False)
        embed.add_field(name=">뮤트역할제거", value=f"`>뮤트` 명령어의 역할을 제거합니다\n`관리자 권한 필요`", inline=False)
        embed.add_field(name=">뮤트", value=f"역할을 추가하여 채팅을 금지시킵니다\n`역할 관리 권한 필요`", inline=False)
        embed.add_field(name=">언뮤트", value=f"역할을 제거하여 채팅금지를 해제합니다\n`역할 관리 권한 필요`", inline=False)
        embed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
    ...
    elif message.content  == '>봇정보':
        infoembed = discord.Embed(title=f"추방 알림", color=0xFFFF00, timestamp=message.created_at)
        infoembed.add_field(name="개발자", value=f"컴하는 루트 [root]#0001", inline=False)
        infoembed.add_field(name="Python Version", value=f"3.9.1", inline=False)
        infoembed.add_field(name="discord.py Version", value=f"1.6.0", inline=False)
        infoembed.add_field(name="서버 수", value=f"{len(client.guilds)}개", inline=False)
        infoembed.add_field(name="Ping", value=f"{round(client.latency * 1000)}", inline=False)
        infoembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
    ...
    if message.content.startswith(">차단해제 "):
        ...
        if message.author.guild_permissions.administrator:
            ...
            file = openpyxl.load_workbook("DB.xlsx")
            sheet = file.active
            i = 1
            while True :
                if sheet["A" + str(i)].value == str(message.guild.id) :
                    errorembed = discord.Embed(title=f"오류 발생", color=0xFF0000, timestamp=message.created_at)
                    errorembed.add_field(name="오류 내용", value=f"역할이 이미 설정되어있습니다\n`>인증역할제거` 명령어를 통해\n설정된 역할을 제거해주세요", inline=False)
                    errorembed.add_field(name="실행자", value=f"{message.author.mention}", inline=False)
                    errorembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                    await message.channel.send(embed=errorembed)
                    break
                    ...
                    sheet["B" + str(i)].value = str(message.author)
                    sheet["c" + str(i)].value = str(captcharole)
                    file.save("DB.xlsx")
            ...
            errorembed = discord.Embed(title=f"오류 발생", color=0xFF0000, timestamp=message.created_at)
            errorembed.add_field(name="오류 내용", value=f"명령어 실행 권한이 없습니다!", inline=False)
            errorembed.add_field(name="실행자", value=f"{message.author.mention}", inline=False)
            errorembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
            await message.channel.send(embed=errorembed)
            return

    elif message.content  == '>인증역할제거':
        if message.author.guild_permissions.administrator:
            file = openpyxl.load_workbook("DB.xlsx")
            sheet = file.active
            i = 1
            while True:
                if sheet["A" + str(i)].value == str(message.guild.id) :
                    ...
                    sheet["C" + str(i)].value = ""
                    file.save("DB.xlsx")
                    successembed = discord.Embed(title=f"역할 제거 완료", color=0x00FF00, timestamp=message.created_at)
                    successembed.add_field(name="제거된 역할:", value=captcharole, inline=False)
                    successembed.add_field(name="실행자", value=f"{message.author.mention}", inline=False)
                    successembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                    await message.channel.send(embed=successembed)
                    break
                    ...
                    file.save("DB.xlsx")
                    errorembed = discord.Embed(title=f"오류 발생", color=0xFF0000, timestamp=message.created_at)
                    errorembed.add_field(name="오류 내용", value=f"역할이 설정되어있지 않습니다", inline=False)
                    errorembed.add_field(name="실행자", value=f"{message.author.mention}", inline=False)
                    errorembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                    await message.channel.send(embed=errorembed)
                    break
...
#            json.dump(contents, f, ensure_ascii=False)
    ...
    if message.content.startswith('>경고 ') :
        if message.author.guild_permissions.view_audit_log:
            try:
                author = await message.guild.fetch_member(int(message.content.split(' ')[1][3:21]))
                reason = message.content[27:]
                file = openpyxl.load_workbook('Warn.xlsx')
                sheet = file.active
                i = 1
                while True :
                    if sheet["A" + str(i)].value == str(author.id) and sheet["B" + str(i)].value == str(message.guild.id):
                        sheet["C" + str(i)].value = int(sheet["C" + str(i)].value) + 1
                        warn = sheet["C" + str(i)].value
                        file.save("Warn.xlsx")
                        if(len(message.content.split(" ")) == 2):
                            reason = "경고 사유가 입력되지 않았습니다!"
                        successembed = discord.Embed(title=f"경고 부여", color=0x00FF00, timestamp=message.created_at)
                        successembed.add_field(name="유저", value=f"<@{author.id}>", inline=False)
                        successembed.add_field(name="경고 사유", value=f"{reason}", inline=False)
                        successembed.add_field(name="총 경고횟수", value=f"{warn}", inline=False)
                        successembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                        await message.channel.send(embed=successembed)
                        break
                    if sheet["A" + str(i)].value == None:
                        sheet["A" + str(i)].value = str(author.id)
                        sheet["B" + str(i)].value = str(message.guild.id)
            ...
            try:
                ...
                while True :
                    if sheet["A" + str(i)].value == str(author.id) and sheet["B" + str(i)].value == str(message.guild.id):
                        ...
                        warn = sheet["C" + str(i)].value
                        file.save("Warn.xlsx")
                        successembed = discord.Embed(title=f"경고 차감", color=0x00FF00, timestamp=message.created_at)
                        successembed.add_field(name="유저", value=f"<@{author.id}>", inline=False)
                        successembed.add_field(name="총 경고횟수", value=f"{warn}", inline=False)
                        successembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                        await message.channel.send(embed=successembed)
                    ...
                    if sheet["A" + str(i)].value == None:
                        warn = sheet["C" + str(i)].value
                        errorembed = discord.Embed(title=f"오류 발생", color=0xFF0000, timestamp=message.created_at)
                        errorembed.add_field(name="오류 내용", value=f"<@{author.id}>님은 경고를 받지 않았습니다", inline=False)
                        errorembed.add_field(name="실행자", value=f"{message.author.mention}", inline=False)
                        errorembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                        await message.channel.send(embed=errorembed)
                        break
        ...
        try:
            ...
            while True :
                if sheet["A" + str(i)].value == str(author.id) and sheet["B" + str(i)].value == str(message.guild.id):
                    warn = sheet["C" + str(i)].value
                    file.save("Warn.xlsx")
                    successembed = discord.Embed(title=f"경고 확인", color=0x00FF00, timestamp=message.created_at)
                    successembed.add_field(name="유저", value=f"<@{author.id}>", inline=False)
                    successembed.add_field(name="총 경고횟수", value=f"{warn}", inline=False)
                    successembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                    await message.channel.send(embed=successembed)
                ...
                if sheet["A" + str(i)].value == None:
                    ...
                    successembed = discord.Embed(title=f"경고 확인", color=0x00FF00, timestamp=message.created_at)
                    successembed.add_field(name="유저", value=f"<@{author.id}>", inline=False)
                    successembed.add_field(name="총 경고횟수", value=f"0", inline=False)
                    successembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                    await message.channel.send(embed=successembed)
                    break
                i += 1
            ...
            try:
                ...
                while True :
                    if sheet["A" + str(i)].value == str(author.id) and sheet["B" + str(i)].value == str(message.guild.id):
                        ...
                        warn = sheet["C" + str(i)].value
                        file.save("Warn.xlsx")
                        successembed = discord.Embed(title=f"경고 초기화", color=0x00FF00, timestamp=message.created_at)
                        successembed.add_field(name="유저", value=f"<@{author.id}>", inline=False)
                        successembed.add_field(name="총 경고횟수", value=f"0", inline=False)
                        successembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                        await message.channel.send(embed=successembed)
    ...
    elif message.content  == '>입장':
        await message.author.voice.channel.connect()
        await message.channel.send("음성 채널에 입장하였습니다")
        
    elif message.content  == '>퇴장':
        for vc in client.voice_clients:
            if vc.guild == message.guild:
                voice = vc
        await voice.disconnect()
        await message.channel.send("음성 채널에서 퇴장하였습니다")
    ...
    if message.content.startswith(">재생"):
        ...
        #ㅁ나ㅗ얼모ㅓㅏㅣㄴㅁㅇㅇㅁㄴ로ㅓㅏㅣㅁㄴ오러ㅏㅣㄴㅇㄹ미ㅘㅓㄴ오ㅓㅏㅣ쿠

    elif message.content.startswith(">뮤트역할 "):
        if message.author.guild_permissions.administrator:
            if str(message.content[6:]) == '':
                errorembed = discord.Embed(title=f"오류 발생", color=0xFF0000, timestamp=message.created_at)
                errorembed.add_field(name="오류 내용", value=f"역할 이름이 입력되지 않았습니다", inline=False)
                ...
                return
            global muterole
            muterole = message.content[6:]
            file = openpyxl.load_workbook("Mute.xlsx")
            sheet = file.active
            i = 1
            while True :
                if sheet["A" + str(i)].value == str(message.guild.id) :
                    ...
                    sheet["A" + str(i)].value = str(message.guild.id)
                    sheet["B" + str(i)].value = str(message.author)
                    sheet["c" + str(i)].value = str(muterole)
                    file.save("Mute.xlsx")
                    successembed = discord.Embed(title=f"역할 설정 완료", color=0x00FF00, timestamp=message.created_at)
                    successembed.add_field(name="설정된 역할:", value=muterole, inline=False)
                    successembed.add_field(name="실행자", value=f"{message.author.mention}", inline=False)
                    successembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                    await message.channel.send(embed=successembed)
                    break
        ...
        if message.author.guild_permissions.administrator:
            ...
            while True:
                if sheet["A" + str(i)].value == str(message.guild.id) :
                    ...
                    file.save("Mute.xlsx")
                    successembed = discord.Embed(title=f"역할 제거 완료", color=0x00FF00, timestamp=message.created_at)
                    successembed.add_field(name="제거된 역할:", value=muterole, inline=False)
                    successembed.add_field(name="실행자", value=f"{message.author.mention}", inline=False)
                    successembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                    await message.channel.send(embed=successembed)
                    break
        ...
        if message.author.guild_permissions.manage_roles:
            try:
                ...
                await author.add_roles(role)
                if(len(message.content.split(" ")) == 2):
                    reason = "뮤트 사유가 입력되지 않았습니다!"
                successembed = discord.Embed(title=f"채팅 금지", color=0x00FF00, timestamp=message.created_at)
                successembed.add_field(name="유저", value=f"<@{author.id}>", inline=False)
                successembed.add_field(name="사유", value=f"{reason}", inline=False)
                successembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                await message.channel.send(embed=successembed)
            except Exception as e:
                if str(e) == "name 'muterole' is not defined":
                    errorembed = discord.Embed(title=f"오류 발생", color=0xFF0000, timestamp=message.created_at)
                    errorembed.add_field(name="오류 내용", value=f"역할이 설정되지 않았습니다\n`>뮤트역할`명령어를 이용하여\n역할을 설정해주세요", inline=False)
                    errorembed.add_field(name="실행자", value=f"{message.author.mention}", inline=False)
                    errorembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
        ...
        if message.author.guild_permissions.manage_roles:
            try:
                ...
                successembed = discord.Embed(title=f"뮤트 해제", color=0x00FF00, timestamp=message.created_at)
                successembed.add_field(name="유저", value=f"<@{author.id}>", inline=False)
                successembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
                await message.channel.send(embed=successembed)
            except Exception as e:
