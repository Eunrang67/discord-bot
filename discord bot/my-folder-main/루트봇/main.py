...
async def on_message(message):
    ...
    elif message.content  == '>업타임':
        uptime = str(Uptime.uptime()).split(":")
        hours = uptime[0]
        minitues = uptime[1]
        seconds = uptimep[2]
        successembed = discord.Embed(title=f"업타임", color=0x00FF00, timestamp=message.created_at)
        successembed.add_field(name="Uptime", value=f"{hours}h {minitues}m {seconds}s", inline=False)
        successembed.set_footer(text=f"{message.author}", icon_url=message.author.avatar_url)
        await message.channel.send(embed=successembed)
